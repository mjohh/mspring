...
int is_coexist_local_brq(struct aps_controller *aps)
{
    ...
    return (is_coexist_brq(w_hi, w_local_brq) && is_coexist_brq(e_hi, e_local_brq) && is_coexist_brq(w_local_brq, e_local_brq));
}
...
int is_coexist_local_brq_and_short_brq(struct aps_controller *aps, enum side local_side)
{
    ...
    return is_coexist_brq(local_hi, local_brq) && SHORT_PATH == DRV_KBYTES_PATH(other_side) && other_hi == other_brq && is_coexist_brq(other_brq, local_brq);
}
...
int is_coexist_short_brq(struct aps_controller *aps)
{
    ...
    if (DRV_KBYTES_PATH(WEST) == SHORT_PATH && DRV_KBYTES_PATH(EAST) == SHORT_PATH) {
        if (!(DRV_KBYTES_DEST(WEST) == MY_NODE_ID && DRV_KBYTES_DEST(EAST) == MY_NODE_ID && DRV_KBYTES_SRC(WEST) == WEST_NODE_ID && DRV_KBYTES_SRC(EAST) == EAST_NODE_ID)) {
            return 0;
        }
        return (w_brq == w_hi && e_brq == e_hi && is_coexist_brq(w_brq, e_brq));
    }
    return 0;
}
...
int is_only_short_brq(struct aps_controller *aps, enum side side)
{
    ...
    if (short_brq > local_brq && short_brq > local_brq_other_side && short_brq > short_brq_other_side && !is_coexist_brq(short_brq, local_brq_other_side) && !is_coexist_brq(short_brq, short_brq_other_side) && (short_brq > long_brq_other_side || is_coexist_brq(short_brq, long_brq_other_side))) {
        return 1;
    }
    
    if (LONG_PATH == DRV_KBYTES_PATH(other_side) && kbrq_for_me_other_side != NR && kbrq_for_me_other_side == hi_brq && !is_coexist_brq(kbrq_for_me_other_side, local_brq) && kbrq_for_me_other_side > local_brq_other_side) {
        return 1;
    }
...
int is_different_span_for_local_sd_ms_exer(struct aps_controller *aps, enum side side)
{
    ...
    return (is_only_local_brq(aps, side) && (local_brq == SD || local_brq == MS || local_brq == EXER) && (DRV_KBYTES_BRCODE(other_side) == SD || DRV_KBYTES_BRCODE(other_side) == MS || DRV_KBYTES_BRCODE(other_side) == EXER) && DRV_KBYTES_PATH(other_side) == LONG_PATH && DRV_KBYTES_DEST(other_side) != MY_NODE_ID);
}
...
int is_different_span_for_short_sd_ms_exer(struct aps_controller *aps, enum side side)
{
    ...
    return (is_only_short_brq(aps, side) && (DRV_KBYTES_BRCODE(side) == SD || DRV_KBYTES_BRCODE(side) == MS || DRV_KBYTES_BRCODE(side) == EXER) && (DRV_KBYTES_BRCODE(other_side) == SD || DRV_KBYTES_BRCODE(other_side) == MS || DRV_KBYTES_BRCODE(other_side) == EXER) && DRV_KBYTES_PATH(other_side) == LONG_PATH && DRV_KBYTES_DEST(other_side) != MY_NODE_ID);
}
