...
int is_only_local_brq(struct aps_controller* aps, enum side local_side) {
	...
	if(NR == local_brq)
		return 0;
	if(SHORT_PATH == other_path && other_hi < local_brq &&
       !is_coexist_brq(other_hi, local_brq) && is_coexist_brq(local_hi, local_brq))
	    return 1;
	if(LONG_PATH == other_path && (other_hi <= local_brq || is_coexist_brq(other_hi, local_brq) /*|| is_ack_for_me*/) &&
       is_coexist_brq(local_hi, local_brq))
	    return 1;
	return 0;
}
