...
void ring_print_kbytes(int nodeid, struct k1k2 *kbytes) {
    ring_print(nodeid, "[%s,%d|%d,%d,%s]n", get_name_of_brq(kbytes->k1.brcode),
               kbytes->k1.dest_node,kbytes->k2.src_node, kbytes->k2.path, get_name_of_status(kbytes->k2.status));
}
